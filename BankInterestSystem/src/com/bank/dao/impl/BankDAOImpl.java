...
public class BankDAOImpl implements BankDAO {
    ...
    private int search(String accountNumber) {
        for (int i = 0; i < list.size(); i++) {
            ...
            String bankAcc = bankAccount.getAccountName();
            //if this is the account number we are looking for return the index
            if (bankAcc.equals(accountNumber)) {
                return i;
            }

        }
        //return -999 if account number does not exist
        return -999;
    }

    @Override
    public int getTotal() {
        //return total number of account currently in the system
        return list.size();
    }
